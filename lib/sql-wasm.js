...
(function(global) {
  ...
  class Database {
    ...
    _execStatement(sql, params) {
      ...
      // Strip leading SQL comment lines (-- ...) before identifying the statement type
      let stripped = sql;
      while (stripped.trimStart().startsWith('--')) {
        stripped = stripped.trimStart();
        const newlineIdx = stripped.indexOf('\n');
        if (newlineIdx === -1) return null; // entire statement is a comment
      ...
      if (upper.startsWith('CREATE TABLE')) {
        return this._createTable(stripped);
      }
      if (upper.startsWith('CREATE TRIGGER')) {
        return this._createTrigger(stripped);
      }
      if (upper.startsWith('CREATE INDEX')) {
        return null; // Indexes are no-ops in this shim
      }
      if (upper.startsWith('INSERT')) {
        return this._insert(stripped, params);
      }
      if (upper.startsWith('SELECT')) {
        return this._select(stripped, params);
      }
      if (upper.startsWith('UPDATE')) {
        return this._update(stripped, params);
      }
      if (upper.startsWith('DELETE')) {
        return this._delete(stripped, params);
      }
